...
We also want to see how these shifts occur among the military branches.

```{r}

change_branches <- military_presence %>%
  filter(US_Or_Overseas == "OS") %>% 
  group_by(Date)%>%
  summarize(
...
change_branches <- change_branches %>%
...
ggplot(data = change_branches) +
  geom_line(aes(x = Date, y = abs_change_army, color = "Army")) +
  geom_line(aes(x = Date, y = abs_change_navy, color = "Navy")) +
  geom_line(aes(x = Date, y = abs_change_marine, color = "Marine Corps")) +
  geom_line(aes(x = Date, y = abs_change_air, color = "Air Force")) +
  geom_line(aes(x = Date, y = abs_change_guard, color = "Coast Guard")) +
...
With this graph, we can see that the greatest shifts in location for military personnel occur among members of the Army, most notably in 2012. This shift of nearly 50,000 members of the Army can be seen in a later question, when we show a nearly 50% change in active military personnel. The next most significant shift was just immediately before 2018, when all military branches some some shift in personnel, with Army and Air Force members seeing the greatest shift.

Otherwise, we can see from this graph that of the branches, Marine Corps most frequently shifts military personnel, especially in the last four years.
...
Djibouti is bordered by Somalia, Eritrea, and Ethiopia. If we compare Djibouti's active U.S. military over time with that of its border countries, we find:
...
ggplot() +
  geom_line(data = military_presence_djibouti, aes(x = Date, y = total_active, color = "Djibouti")) +
  geom_line(data = military_presence_somalia, aes(x = Date, y = total_active, color = "Somalia")) +
  geom_line(data = military_presence_eritrea, aes(x = Date, y = total_active, color = "Eritrea")) +
  geom_line(data = military_presence_ethiopia, aes(x = Date, y = total_active, color = "Ethiopia")) +
...
You can see that Djibouti's numbers are significantly higher than its border countries.
